<script setup lang="ts">
  ...
  const benchmarks = ref(Mapper.instance.getBenchmarksForValidation() || []);
      ...
      <div class="container mx-auto px-6">
        <div class="grid lg:grid-cols-12 gap-8 mb-20">
          <motion.div
            ...
            class="lg:col-span-4"
          >
            <span class="tag text-muted-foreground mb-4 block w-fit">{{ Mapper.instance.validationBenchmarks?.packages.length ?? 0 }} packages</span>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
        	<motion.a
            :href="'/validation/' + result.package.toLowerCase()"  
        	  :key="result.package"
            ...
            <div class="flex flex-col gap-2">
              ...
              <span class="font-mono text-xs text-muted-foreground/50 flex justify-between items-center results">
                OPS/S (FLAT_OBJECT)
                <span class="ml-1">{{ result.benchmarks[0].avgScore.toFixed(2) }}</span>
              </span>
              <div class="w-full h-2 bg-border rounded-full overflow-hidden mt-1">
                <div class="h-full bg-primary rounded-full" :style="{ width: result.benchmarks[0].avgScorePercentage + '%' }"></div>
              </div>
            </div>
        	</motion.a>
        </div>
		    <a href="/validation" class="inline-flex items-center gap-2 text-sm text-muted-foreground! hover:text-primary! transition-colors group">
